...
const serviceCopy: Record<
  ServiceId,
  { name: { en: string; es: string }; description: { en: string; es: string } }
> = {
  standard: {
...
const frequencyCopy: Record<
  FrequencyId,
  { name: { en: string; es: string }; note: { en: string; es: string } }
> = {
  onetime: {
    name: { en: "One-time", es: "Una vez" },
    note: { en: "No commitment", es: "Sin compromiso" },
  },
  weekly: {
    name: { en: "Weekly", es: "Semanal" },
    note: { en: "20% savings", es: "20% de ahorro" },
  },
  biweekly: {
    name: { en: "Bi-weekly", es: "Cada dos semanas" },
    note: { en: "15% savings", es: "15% de ahorro" },
  },
  monthly: {
    name: { en: "Monthly", es: "Mensual" },
    note: { en: "10% savings", es: "10% de ahorro" },
  },
};
...
export function BookingFlow({ initialService }: { initialService?: ServiceId }) {
  ...
  function goNext() {
    if (step === 2 && pets === "yes" && petDetails.trim().length < 2) {
      setPetError(
        text({
          en: "Please add a brief note about your pet or pets.",
          es: "Agrega una nota breve sobre tu mascota o mascotas.",
        }),
      );
      return;
    }
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_21rem] lg:items-start">
      <div className="min-w-0">
        <ol
          className="mb-8 flex flex-wrap gap-x-4 gap-y-2 text-xs"
          aria-label={text({ en: "Service request progress", es: "Progreso de la solicitud" })}
        >
          {steps.map((label, index) => (
            ...
            >
              <span
                className={`inline-flex size-6 items-center justify-center rounded-full border text-[0.7rem] ${index < step ? "border-moss bg-moss text-primary-foreground" : index === step ? "border-moss bg-accent" : "border-border bg-card"}`}
              >
        ...
        {step === 0 && (
          <fieldset>
            <legend className="text-2xl">
              {text({ en: "Choose your cleaning type", es: "Elige tu tipo de limpieza" })}
            </legend>
            <p className="mt-2 text-sm text-muted-foreground">
              {text({
                en: "Published pricing is based on a standard average 1-bedroom, 1-full-bath home.",
                es: "Los precios publicados se basan en una vivienda estándar promedio de 1 dormitorio y 1 baño completo.",
              })}
            </p>
                  ...
                  <span className="flex items-start justify-between gap-4">
                    <span>
                      <span className="block text-lg font-semibold text-ink">
                        {text(serviceCopy[item.id].name)}
                      </span>
                      <span className="mt-1 block text-sm leading-relaxed text-muted-foreground">
                        {text(serviceCopy[item.id].description)}
                      </span>
                    </span>
        ...
        {step === 1 && (
          <fieldset>
            <legend className="text-2xl">
              {text({
                en: "How often would you like service?",
                es: "¿Con qué frecuencia deseas el servicio?",
              })}
            </legend>
            <p className="mt-2 text-sm text-muted-foreground">
              {text({
                en: "Recurring savings apply to the service subtotal only. Add-ons remain at their listed rate.",
                es: "Los descuentos recurrentes se aplican solo al subtotal del servicio. Los servicios adicionales conservan su precio publicado.",
              })}
            </p>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {availableFrequencies(service).map((item) => (
                <button
                  ...
                  className={`rounded-xl border p-5 text-left transition-colors ${frequency === item.id ? "border-moss bg-accent/45" : "border-border bg-card hover:border-moss/60"}`}
                >
                  <span className="block text-lg font-semibold text-ink">
                    {text(frequencyCopy[item.id].name)}
                  </span>
                  <span className="mt-1 block text-xs text-moss">
                    {text(frequencyCopy[item.id].note)}
                  </span>
                  <span className="mt-4 block font-display text-3xl text-ink">
                    {money(servicePrice(service, item.id))}
                  </span>
                  <span className="text-xs text-muted-foreground">
                    {text({ en: "base service per visit", es: "servicio base por visita" })}
                  </span>
                </button>
              ))}
            </div>
        ...
        {step === 2 && (
          <div>
            <h2 className="text-2xl">
              {text({
                en: "Your home and cleaning scope",
                es: "Tu hogar y el alcance de la limpieza",
              })}
            </h2>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              {text({
                en: `Base pricing includes ${BASE_BEDROOMS} bedroom and ${BASE_FULL_BATHS} full bathroom. Additional room charges are derived from the quantities below so they are counted once.`,
                es: `El precio base incluye ${BASE_BEDROOMS} dormitorio y ${BASE_FULL_BATHS} baño completo. Los cargos por habitaciones adicionales se calculan con las cantidades siguientes para evitar cargos duplicados.`,
              })}
            </p>

            <div className="mt-6 grid gap-3">
              <QuantityField
                label={text({ en: "Bedrooms to clean", es: "Dormitorios a limpiar" })}
                value={scope.bedrooms}
                min={1}
                onChange={(value) => setScope((current) => ({ ...current, bedrooms: value }))}
                hint={text({ en: "1 included in base price", es: "1 incluido en el precio base" })}
                price={`+${money(addOnPrice(getAddOn("extra-bedroom"), service))} ${text({ en: "each additional", es: "cada adicional" })}`}
              />
              <QuantityField
                label={text({ en: "Full bathrooms to clean", es: "Baños completos a limpiar" })}
                value={scope.fullBaths}
                min={1}
                onChange={(value) => setScope((current) => ({ ...current, fullBaths: value }))}
                hint={text({ en: "1 included in base price", es: "1 incluido en el precio base" })}
                price={`${text({ en: "starting at", es: "desde" })} +${money(addOnPrice(getAddOn("extra-full-bath"), service))} ${text({ en: "each additional", es: "cada adicional" })}`}
              />
              <QuantityField
                label={text({ en: "Half bathrooms", es: "Medios baños" })}
                value={scope.halfBaths}
                onChange={(value) => setScope((current) => ({ ...current, halfBaths: value }))}
                price={`+${money(addOnPrice(getAddOn("half-bath"), service))} ${text({ en: "each", es: "cada uno" })}`}
              />
              <QuantityField
                label={text({ en: "Additional living rooms", es: "Salas adicionales" })}
                value={scope.livingRooms}
                onChange={(value) => setScope((current) => ({ ...current, livingRooms: value }))}
                price={`+${money(addOnPrice(getAddOn("living-room"), service))} ${text({ en: "each", es: "cada una" })}`}
              />
              <QuantityField
                label={text({ en: "Dining rooms", es: "Comedores" })}
                value={scope.diningRooms}
                onChange={(value) => setScope((current) => ({ ...current, diningRooms: value }))}
                price={`+${money(addOnPrice(getAddOn("dining-room"), service))} ${text({ en: "each", es: "cada uno" })}`}
              />
              <QuantityField
                label={text({ en: "Offices", es: "Oficinas" })}
                value={scope.offices}
                onChange={(value) => setScope((current) => ({ ...current, offices: value }))}
                price={`+${money(addOnPrice(getAddOn("office"), service))} ${text({ en: "each", es: "cada una" })}`}
              />
              <QuantityField
                label={text({
                  en: "Laundry or utility rooms",
                  es: "Lavanderías o cuartos de servicio",
                })}
                value={scope.laundryRooms}
                onChange={(value) => setScope((current) => ({ ...current, laundryRooms: value }))}
                price={`+${money(addOnPrice(getAddOn("laundry-room"), service))} ${text({ en: "each", es: "cada uno" })}`}
              />
            </div>

            <div className="mt-7 grid gap-6 md:grid-cols-2">
              <div>
                <Label htmlFor="booking-sqft">
                  {text({
                    en: "Approximate square footage, for review only",
                    es: "Pies cuadrados aproximados, solo para revisión",
                  })}
                </Label>
                <Input
                  id="booking-sqft"
                  ...
                  placeholder={text({ en: "Example: 1800", es: "Ejemplo: 1800" })}
                  className="mt-2"
                />
                <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                  {text({
                    en: `We do not price by square footage. Around ${CUSTOM_REVIEW_SQFT.toLocaleString()} sq ft and above requires custom review.`,
                    es: `No calculamos el precio por pies cuadrados. Aproximadamente ${CUSTOM_REVIEW_SQFT.toLocaleString()} pies cuadrados o más requiere una revisión personalizada.`,
                  })}
                </p>
              </div>
              <label className="flex min-h-24 cursor-pointer items-start gap-3 rounded-xl border border-border bg-card p-4">
                ...
                <span>
                  <span className="block text-sm font-semibold text-ink">
                    {text({
                      en: "I only want part of the home cleaned",
                      es: "Solo quiero limpiar parte del hogar",
                    })}
                  </span>
                  <span className="mt-1 block text-xs leading-relaxed text-muted-foreground">
                    {text({
                      en: "Partial-home requests are reviewed as custom scope.",
                      es: "Las solicitudes para limpiar solo parte del hogar se revisan como un alcance personalizado.",
                    })}
                  </span>
                </span>
              </label>
            ...
            <div className="mt-7 grid gap-5 md:grid-cols-2">
              <fieldset>
                <legend className="text-sm font-semibold text-ink">
                  {text({ en: "Pets in the home?", es: "¿Hay mascotas en el hogar?" })}
                </legend>
                <div className="mt-3 flex gap-3">
                  {(["no", "yes"] as const).map((value) => (
                    <button
                      ...
                      className={`min-h-11 rounded-full border px-5 text-sm font-semibold ${pets === value ? "border-moss bg-accent text-ink" : "border-border bg-card text-muted-foreground"}`}
                    >
                      {value === "yes"
                        ? text({ en: "Yes", es: "Sí" })
                        : text({ en: "No", es: "No" })}
                ...
                {pets === "yes" && (
                  <div className="mt-4">
                    <Label htmlFor="pet-details">
                      {text({
                        en: "Pet type and anything we should know",
                        es: "Tipo de mascota y cualquier detalle importante",
                      })}
                    </Label>
                    <Input
                      id="pet-details"
                      ...
                      className="mt-2"
                    />
                    {petError && <p className="mt-1 text-xs text-destructive">{petError}</p>}
                  </div>
                )}
              </fieldset>
              <div>
                <Label htmlFor="other-spaces">
                  {text({ en: "Other or special spaces", es: "Otros espacios o áreas especiales" })}
                </Label>
                <Input
                  id="other-spaces"
                  value={otherSpaces}
                  ...
                  placeholder={text({
                    en: "Example: playroom, stairs, sunroom",
                    es: "Ejemplo: sala de juegos, escaleras, terraza cerrada",
                  })}
                  className="mt-2"
                />
              </div>
            ...
            <div className="mt-6">
              <Label htmlFor="service-notes">
                {text({
                  en: "Condition or service notes",
                  es: "Notas sobre la condición o el servicio",
                })}
              </Label>
              <Textarea
                id="service-notes"
                ...
                placeholder={text({
                  en: "Share anything that will help us understand the space.",
                  es: "Comparte cualquier detalle que nos ayude a entender el espacio.",
                })}
        ...
        {step === 3 && (
          <div>
            <h2 className="text-2xl">
              {text({
                en: "Choose optional add-ons",
                es: "Elige servicios adicionales opcionales",
              })}
            </h2>
            <p className="mt-2 text-sm text-muted-foreground">
              {text({
                en: "Room charges from the previous step are calculated automatically. Use this step only for optional detail work.",
                es: "Los cargos por habitaciones del paso anterior se calculan automáticamente. Usa este paso solo para trabajos detallados opcionales.",
              })}
            </p>
            <div className="mt-6 grid gap-3">
              {selectableAddOns.map((addOn) => {
                ...
                return (
                  <div
                    key={addOn.id}
                    className={`rounded-xl border p-4 ${qty > 0 ? "border-moss bg-accent/35" : "border-border bg-card"}`}
                  >
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="text-sm font-semibold text-ink">{label}</p>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {addOn.startingAt ? text({ en: "Starting at ", es: "Desde " }) : ""}
                          {money(price)}
                          {addOn.unit
                            ? ` ${text({ en: `per ${addOn.unit}`, es: addOn.unit === "load" ? "por carga" : `por ${addOn.unit}` })}`
                            : ""}
                        </p>
                      </div>
                      {addOn.quantity ? (
                        ...
                        <button
                          type="button"
                          onClick={() => setExtra(addOn.id, qty ? 0 : 1)}
                          aria-pressed={qty > 0}
                          className={`min-h-10 rounded-full border px-4 text-xs font-semibold ${qty > 0 ? "border-moss bg-moss text-primary-foreground" : "border-border text-ink"}`}
                        >
                          {qty > 0
                            ? text({ en: "Added", es: "Agregado" })
                            : text({ en: "Add", es: "Agregar" })}
                        </button>
                      )}
                    </div>
                  </div>
        ...
        {step === 4 && (
          <div>
            <h2 className="text-2xl">
              {text({
                en: "Schedule preference and contact",
                es: "Preferencia de horario y contacto",
              })}
            </h2>
            <p className="mt-2 text-sm text-muted-foreground">
              {text({
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <Field label={text({ en: "Full name", es: "Nombre completo" })} error={errors.name}>
                <Input
                  value={contact.name}
              ...
              <Field label={text({ en: "Email", es: "Correo electrónico" })} error={errors.email}>
                <Input
                  type="email"
                  value={contact.email}
              ...
              <Field label={text({ en: "Phone", es: "Teléfono" })} error={errors.phone}>
                <Input
                  type="tel"
                  value={contact.phone}
              ...
              <Field label={text({ en: "Street address", es: "Dirección" })} error={errors.address}>
                <Input
              ...
              <Field label={text({ en: "City", es: "Ciudad" })} error={errors.city}>
                <Input
                  value={contact.city}
              ...
              <Field label={text({ en: "ZIP code", es: "Código postal" })} error={errors.zip}>
                <Input
                  value={contact.zip}
                  onChange={(event) =>
                    updateContact("zip", event.target.value.replace(/\D/g, "").slice(0, 5))
                  }
                  inputMode="numeric"
        ...
        {step === 5 && (
          <div>
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
              <ReviewCard
                title={text({ en: "Service", es: "Servicio" })}
                lines={[
                  serviceLabel,
                  frequencyLabel,
                  `${scope.bedrooms} ${text({ en: "bedroom(s)", es: "dormitorio(s)" })}`,
                  `${scope.fullBaths} ${text({ en: "full bath(s)", es: "baño(s) completo(s)" })}`,
                ]}
              />
              <ReviewCard
                title={text({ en: "Contact", es: "Contacto" })}
                lines={[
                  contact.name,
                  contact.email,
                  contact.phone,
                  `${contact.address}, ${contact.city}, TX ${contact.zip}`,
                  `${contact.date} · ${language === "es" ? selectedWindow?.es : selectedWindow?.en}`,
                ]}
              />
            </div>
            ...
            {reviewItems.length > 0 && (
              <div className="mt-6 rounded-xl border border-moss/35 bg-accent/35 p-5">
                <div className="flex items-center gap-2 text-sm font-semibold text-ink">
                  <Info className="size-4 text-moss" aria-hidden="true" />
                  {text({ en: "Review needed", es: "Se requiere revisión" })}
                </div>
                ...
                <Button asChild variant="outline" size="sm" className="mt-4">
                  <Link to="/quote">
                    {text({
                      en: "Open custom quote instead",
                      es: "Abrir cotización personalizada",
                    })}
                  </Link>
                </Button>
              </div>
      ...
      <aside className="rounded-2xl border border-gold/20 bg-card/80 p-5 shadow-lift lg:sticky lg:top-32">
        ...
        <div className="mt-5 space-y-3 border-y border-border py-4 text-sm">
          ...
          {estimate.discountAmount > 0 && (
            <SummaryRow
              label={text({ en: "Recurring savings", es: "Ahorro recurrente" })}
              value={`-${money(estimate.discountAmount)}`}
            />
          )}
          <SummaryRow
            label={text({ en: "Service subtotal", es: "Subtotal del servicio" })}
            value={money(estimate.serviceSubtotal)}
          />
          <SummaryRow
            label={text({ en: "Rooms and add-ons", es: "Habitaciones y adicionales" })}
            value={money(estimate.addOnTotal)}
          />
        </div>
        <div className="mt-5 flex items-end justify-between gap-4">
          <span className="text-sm font-semibold text-ink">
            {text({ en: "Estimated total", es: "Total estimado" })}
          </span>
          <span className="font-display text-4xl text-ink">{money(estimate.total)}</span>
        </div>
        {estimate.hasStartingAt && (
          <p className="mt-2 text-xs text-moss">
            {text({
              en: "Includes one or more starting-at items.",
              es: "Incluye uno o más servicios con precio inicial.",
            })}
          </p>
        )}
        <p className="mt-5 text-xs leading-relaxed text-muted-foreground">
          {text({
            en: "This is an estimate, not a confirmed appointment or final invoice. Final service details and pricing are confirmed before cleaning.",
            es: "Este es un estimado, no una cita confirmada ni una factura final. Los detalles y el precio final se confirman antes de la limpieza.",
          })}
        </p>
      </aside>
    </div>
