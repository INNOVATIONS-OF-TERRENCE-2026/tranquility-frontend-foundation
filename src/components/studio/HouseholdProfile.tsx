...
const householdToggles: { key: keyof HouseholdProfileType; label: string; note: string }[] = [
  { key: "childrenPresent", label: "Children present", note: "Helps us understand household traffic and room use." },
  { key: "workFromHome", label: "Work-from-home household", note: "Useful for planning around occupied work areas." },
  { key: "highTraffic", label: "High-traffic home", note: "Flags frequently used spaces for review." },
  { key: "shoesOff", label: "Shoes-off home", note: "A household preference we can respect." },
  { key: "frequentEntertaining", label: "Frequent entertaining", note: "Adds context about common-area use." },
  { key: "shortTermRental", label: "Short-term rental context", note: "May require a custom scope discussion." },
  { key: "accessibilityConsiderations", label: "Accessibility considerations", note: "Share only what is useful for safe service planning." },
  { key: "fragileObjects", label: "Fragile objects", note: "Flags spaces that may need additional review." },
  { key: "plants", label: "Plants", note: "Useful context for occupied surfaces and room layout." },
  { key: "collections", label: "Collections or displays", note: "Helps identify items that should not be disturbed." },
  { key: "specialtyFurnishings", label: "Specialty furnishings", note: "May require custom handling instructions." },
];
...
export function HouseholdProfile({
  ...
  function toggleBoolean(key: keyof HouseholdProfileType) {
    const value = household[key];
    if (typeof value !== "boolean") return;
    onHouseholdChange({ ...household, [key]: !value });
  }
  ...
  return (
    <div>
      ...
      <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground">
        These details are optional. Share only what helps Tranquility understand how your home is used and what should be handled carefully.
      </p>

      <section className="mt-8 rounded-xl border border-border bg-card p-5 shadow-soft" aria-labelledby="pets-heading">
        <h3 id="pets-heading" className="text-xl">Pets</h3>
          ...
          {([false, true] as const).map((value) => (
            <button
              ...
              className={`min-h-11 min-w-24 rounded-md border px-4 py-2 text-sm font-semibold ${
                household.pets === value ? "border-moss bg-accent text-accent-foreground" : "border-border bg-background"
              }`}
            >
              {value ? "Yes" : "No"}
            </button>
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            <div>
              ...
              <Input
                ...
                maxLength={180}
                onChange={(event) => onHouseholdChange({ ...household, petTypes: event.target.value })}
      ...
      <fieldset className="mt-8">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {householdToggles.map((item) => {
            const active = household[item.key] === true;
            return (
              <label
                key={item.key}
                className={`cursor-pointer rounded-xl border p-4 ${active ? "border-moss bg-accent/45" : "border-border bg-card"}`}
              >
      ...
      <div className="mt-6">
        ...
        <Textarea
          ...
          maxLength={700}
          onChange={(event) => onHouseholdChange({ ...household, notes: event.target.value })}
          placeholder="Anything useful about access, occupied rooms, pets, fragile areas, or household routines"
          className="mt-2 min-h-28"
        />
      </div>

      <section className="mt-10 rounded-2xl border border-oak/50 bg-sand p-6" aria-labelledby="protected-heading">
        <div className="flex items-start gap-3">
          <ShieldCheck className="mt-0.5 size-5 shrink-0 text-oak" aria-hidden="true" />
          <div>
            <p className="eyebrow">Protected areas</p>
            <h3 id="protected-heading" className="mt-2 text-2xl">Please Leave These Areas Alone</h3>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
              Mark anything you do not want moved, opened, cleaned, or disturbed. These instructions flow directly into the Home Care Blueprint.
            </p>
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  className={`min-h-10 rounded-full border px-3 py-2 text-xs font-medium ${
                    active ? "border-oak bg-card text-ink" : "border-border bg-background text-muted-foreground"
                  }`}
                >
                  {item}
                </button>
