      ...
      <div className="grid lg:grid-cols-[22rem_minmax(0,1fr)]">
        <div className="border-b border-border bg-sand/70 p-5 lg:border-b-0 lg:border-r lg:p-6">
          <div className="flex items-center gap-2 text-moss">
            ...
            <p className="text-xs font-bold uppercase tracking-[0.16em]">Interactive service map</p>
          </div>
          <h2 className="mt-3 text-2xl">Find your DFW city</h2>
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
            Search the listed service cities, choose a planning radius, and see which listed communities sit near your selected city.
          </p>
          ...
          <div className="relative mt-6">
            <button
              ...
              aria-expanded={open}
              className="flex min-h-12 w-full items-center justify-between gap-3 rounded-2xl border border-input bg-card px-4 text-left text-sm font-semibold text-ink shadow-soft"
            >
                ...
                <div className="relative border-b border-border p-3">
                  ...
                  {search && (
                    <button
                      ...
                      className="absolute right-5 top-1/2 inline-flex size-8 -translate-y-1/2 items-center justify-center rounded-full text-muted-foreground hover:bg-accent hover:text-ink"
                    >
                ...
                <div className="max-h-64 overflow-y-auto p-2">
                  {matchingCities.map((city) => (
                    <button
                      ...
                      className={`flex min-h-11 w-full items-center justify-between rounded-xl px-3 text-left text-sm transition ${city.name === selected.name ? "bg-accent font-semibold text-accent-foreground" : "text-foreground hover:bg-muted"}`}
                    >
                      {city.name}
                      {city.name === selected.name && <span className="text-[0.65rem] font-bold uppercase tracking-wider">Selected</span>}
                    </button>
                  ))}
                  {matchingCities.length === 0 && (
                    <p className="px-3 py-5 text-sm text-muted-foreground">No listed city matches that search.</p>
                  )}
                </div>
              </div>
            )}
          ...
          <label className="mt-5 block text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground" htmlFor="service-radius">
            Planning radius
          </label>
          <select
            id="service-radius"
            value={radius}
            onChange={(event) => setRadius(Number(event.target.value) as (typeof radiusOptions)[number])}
            className="mt-2 min-h-12 w-full rounded-2xl border border-input bg-card px-4 text-sm font-semibold text-ink shadow-soft outline-none focus:border-ring"
          >
          ...
          <div className="mt-6 rounded-2xl border border-border bg-card p-4">
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-moss">Within {radius} miles of {selected.name}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {nearbyCities.map(({ city, miles }) => (
                <button
                  ...
                  className="rounded-full border border-border bg-background px-3 py-1.5 text-xs text-foreground hover:border-ring hover:text-ink"
                >
                  {city.name} <span className="text-muted-foreground">{miles < 1 ? "local" : `${Math.round(miles)} mi`}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
            Radius results are a planning view of the listed cities, not a guarantee of service at every address. Exact coverage is confirmed when a customer submits an address.
          </p>
        </div>
        ...
        <div className="relative min-h-[30rem] bg-muted lg:min-h-[38rem]">
          <iframe
            key={selected.name}
            title={`Map centered on ${selected.name}, Texas`}
            src={mapUrl}
            loading="lazy"
          ...
          <div className="pointer-events-none absolute left-4 top-4 rounded-2xl border border-white/20 bg-black/70 px-4 py-3 text-white shadow-lift backdrop-blur-md">
            <p className="text-[0.62rem] font-bold uppercase tracking-[0.16em] text-white/65">Map center</p>
