      ...
      <div className="grid lg:grid-cols-[22rem_minmax(0,1fr)]">
        <div className="border-b border-border bg-sand/70 p-5 lg:border-b-0 lg:border-r lg:p-6">
          <div className="flex items-center gap-2 text-moss">
            ...
            <p className="text-xs font-bold uppercase tracking-[0.16em]">
              {text({ en: "Interactive service map", es: "Mapa interactivo de servicio" })}
            </p>
          </div>
          <h2 className="mt-3 text-2xl">
            {text({ en: "Find your DFW city", es: "Encuentra tu ciudad en DFW" })}
          </h2>
              ...
              <div className="absolute inset-x-0 top-[calc(100%+0.5rem)] z-30 overflow-hidden rounded-2xl border border-border bg-popover shadow-lift">
                <div className="relative border-b border-border p-3">
                  ...
                  <input
                    ...
                    aria-label={text({
                      en: "Search service cities",
                      es: "Buscar ciudades de servicio",
                    })}
                  ...
                  {search && (
                    <button
                      type="button"
                      onClick={() => setSearch("")}
                      aria-label={text({ en: "Clear city search", es: "Borrar búsqueda" })}
                ...
                <div className="max-h-64 overflow-y-auto p-2">
                    ...
                    >
                      ...
                      {city.name === selected.name && (
                        <span className="text-[0.65rem] font-bold uppercase tracking-wider">
                          {text({ en: "Selected", es: "Seleccionada" })}
                        </span>
                      )}
                    </button>
                  ))}
                  {matchingCities.length === 0 && (
                    <p className="px-3 py-5 text-sm text-muted-foreground">
                      {text({
                        en: "No listed city matches that search.",
                        es: "Ninguna ciudad de la lista coincide con esa búsqueda.",
                      })}
                    </p>
                  )}
                </div>
          ...
          <label
            className="mt-5 block text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground"
            htmlFor="service-radius"
          >
            {text({ en: "Planning radius", es: "Radio de planificación" })}
          </label>
          <select
            id="service-radius"
            value={radius}
            onChange={(event) =>
              setRadius(Number(event.target.value) as (typeof radiusOptions)[number])
            }
          ...
          >
            {radiusOptions.map((option) => (
              <option key={option} value={option}>
                {option} {text({ en: "miles", es: "millas" })}
              </option>
            ))}
          </select>
          ...
          <div className="mt-6 rounded-2xl border border-border bg-card p-4">
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-moss">
              {text({
                en: `Within ${radius} miles of ${selected.name}`,
                es: `Dentro de ${radius} millas de ${selected.name}`,
              })}
            </p>
              ...
              {nearbyCities.map(({ city, miles }) => (
                ...
                >
                  ...
                  <span className="text-muted-foreground">
                    {miles < 1 ? text({ en: "local", es: "local" }) : `${Math.round(miles)} mi`}
                  </span>
                </button>
              ))}
            </div>
        ...
        <div className="relative min-h-[30rem] bg-muted lg:min-h-[38rem]">
          <iframe
            key={selected.name}
            title={text({
              en: `Map centered on ${selected.name}, Texas`,
              es: `Mapa centrado en ${selected.name}, Texas`,
            })}
            src={mapUrl}
            loading="lazy"
          ...
          <div className="pointer-events-none absolute left-4 top-4 rounded-2xl border border-white/20 bg-black/70 px-4 py-3 text-white shadow-lift backdrop-blur-md">
            <p className="text-[0.62rem] font-bold uppercase tracking-[0.16em] text-white/65">
              {text({ en: "Map center", es: "Centro del mapa" })}
            </p>
