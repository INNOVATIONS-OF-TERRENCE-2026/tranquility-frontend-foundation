      ...
      <section className="section">
        <div className="container-page grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
          <div>
            ...
            {notes && (
              <p className="mt-8 rounded-lg border border-border bg-muted p-5 text-sm leading-relaxed text-muted-foreground">
                {notes}
              </p>
            )}
          </div>
                ...
                .map((f) => (
                  <div key={f.id} className="flex items-baseline justify-between gap-3">
                    <dt className="text-muted-foreground">
                      {f.name} <span className="text-xs text-moss">({f.note})</span>
                    </dt>
