...
function QuotePage() {
  ...
  const propertyTypes: { id: PropertyType; label: string; note: string }[] = [
    {
      ...
      note: text({
        en: "Large, unusual, or partial-home scope",
        es: "Viviendas grandes, inusuales o limpieza parcial",
      }),
    },
    {
      id: "commercial",
      ...
      note: text({
        en: "Suites, offices, light commercial",
        es: "Suites, oficinas y espacios comerciales ligeros",
      }),
    },
    {
      id: "specialty",
      ...
      note: text({
        en: "Post-renovation, unique surfaces, or other needs",
        es: "Después de renovación, superficies especiales u otras necesidades",
      }),
    },
  ];
  ...
  function addPhotos(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      if (photos.length + next.length >= MAX_PHOTOS) {
        messages.push(
          text({ en: `Maximum ${MAX_PHOTOS} photos.`, es: `Máximo de ${MAX_PHOTOS} fotos.` }),
        );
        break;
      }
      ...
      if (
        ...
        next.some((photo) => photo.fingerprint === fingerprint)
      ) {
        messages.push(
          text({
            en: `${file.name} is already selected.`,
            es: `${file.name} ya está seleccionada.`,
          }),
        );
        continue;
      }
      if (!isAcceptedImage(file)) {
        messages.push(
          text({
            en: `${file.name} is not a supported image type.`,
            es: `${file.name} no es un tipo de imagen compatible.`,
          }),
        );
        continue;
      }
      if (file.size > MAX_PHOTO_BYTES) {
        messages.push(
          text({ en: `${file.name} is larger than 8 MB.`, es: `${file.name} supera los 8 MB.` }),
        );
        continue;
      }
      if (totalBytes + file.size > MAX_TOTAL_BYTES) {
        messages.push(
          text({
            en: "Selected photos would exceed the 50 MB total limit.",
            es: "Las fotos seleccionadas superarían el límite total de 50 MB.",
          }),
        );
        break;
      }
  ...
  function validate() {
    ...
    if (values.scope.trim().length < 10)
      next.scope = text({
        en: "Tell us a little more about the scope",
        es: "Cuéntanos un poco más sobre el alcance",
      });
    setErrors(next);
  ...
  return (
    <>
      <PageHero
        ...
        title={text({
          en: "Get a custom quote or virtual consultation",
          es: "Obtén una cotización personalizada o una consulta virtual",
        })}
        intro={text({
        ...
        <div className="container-page grid gap-10 lg:grid-cols-[1fr_18rem] lg:items-start">
          ...
          >
            <fieldset>
              <legend className="text-lg">
                {text({ en: "What kind of property?", es: "¿Qué tipo de propiedad es?" })}
              </legend>
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                {propertyTypes.map((property) => (
                  <button
                    ...
                    className={`min-h-24 rounded-lg border p-4 text-left transition-colors ${values.propertyType === property.id ? "border-moss bg-accent/40" : "border-border bg-card hover:border-moss/50"}`}
                  >
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <TextField
                id="q-name"
                label={text({ en: "Full name", es: "Nombre completo" })}
                value={values.name}
                error={errors.name}
              ...
              <TextField
                id="q-email"
                label={text({ en: "Email", es: "Correo electrónico" })}
                type="email"
                value={values.email}
                error={errors.email}
              ...
              <TextField
                id="q-phone"
                label={text({ en: "Phone", es: "Teléfono" })}
                type="tel"
                value={values.phone}
                error={errors.phone}
              ...
              <TextField
                id="q-size"
                label={text({
                  en: "Approximate size (optional)",
                  es: "Tamaño aproximado (opcional)",
                })}
                value={values.size}
                error={errors.size}
                onChange={(value) => set("size", value)}
                placeholder={text({
                  en: "e.g. 3,200 sq ft, 4 bed / 4 bath",
                  es: "Ej. 3,200 pies², 4 dormitorios / 4 baños",
                })}
              />
              <TextField
                id="q-timing"
                label={text({
                  en: "Desired timing (optional)",
                  es: "Fecha o periodo deseado (opcional)",
                })}
                value={values.timing}
                error={errors.timing}
                onChange={(value) => set("timing", value)}
                placeholder={text({ en: "e.g. within two weeks", es: "Ej. dentro de dos semanas" })}
              />
            </div>

            <div>
              <Label htmlFor="q-scope">
                {text({ en: "Scope & condition", es: "Alcance y condición" })}
              </Label>
              <Textarea
                id="q-scope"
                value={values.scope}
                ...
                placeholder={text({
                  en: "Which rooms or areas, how the space is used, current condition, and anything specialized",
                  es: "Qué habitaciones o áreas, cómo se usa el espacio, su condición actual y cualquier necesidad especial",
                })}
            ...
            <fieldset>
              <legend className="text-sm font-medium text-ink">
                {text({
                  ...
                  es: "¿Cómo prefieres que nos comuniquemos contigo?",
                })}
              </legend>
              <div className="mt-3 flex flex-wrap gap-2">
                {contactPrefs.map((preference) => (
                  <button
                    ...
                    className={`min-h-11 rounded-md border px-4 py-2.5 text-sm transition-colors ${values.contactPreference === preference.id ? "border-moss bg-accent/50" : "border-border bg-card hover:border-moss/50"}`}
                  >
                    {preference.label}
                  </button>
                ))}
            ...
            <div>
              <Label htmlFor="q-notes">
                {text({ en: "Additional notes (optional)", es: "Notas adicionales (opcional)" })}
              </Label>
              <Textarea
                id="q-notes"
                value={values.notes}
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              <h2 className="text-lg">
                {text({ en: "Photos (optional)", es: "Fotos (opcional)" })}
              </h2>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <Button
                  ...
                  disabled={photos.length >= MAX_PHOTOS || selectedBytes >= MAX_TOTAL_BYTES}
                >
                  <ImagePlus className="size-4" aria-hidden="true" />
                  {photos.length >= MAX_PHOTOS
                    ? text({ en: "Photo limit reached", es: "Límite de fotos alcanzado" })
                    : text({ en: "Choose photos", es: "Elegir fotos" })}
                </Button>
                <span className="text-xs text-muted-foreground">
                  {photos.length}/{MAX_PHOTOS} · {formatBytes(selectedBytes)} / 50 MB
                </span>
              </div>
              {errors.photos && (
                    ...
                    >
                      {photo.url ? (
                        <img
                          src={photo.url}
                          alt={text({
                            en: `Selected photo ${index + 1}: ${photo.name}`,
                            es: `Foto seleccionada ${index + 1}: ${photo.name}`,
                          })}
                          className="aspect-square w-full object-cover"
                        />
                      ) : (
                        <div className="flex aspect-square items-center justify-center p-4 text-center text-xs text-muted-foreground">
                          {photo.name}
                          <br />
                          {text({
                            en: "Preview not supported by this browser",
                            es: "Este navegador no admite la vista previa",
                          })}
                        </div>
                      )}
                      <button
                        ...
                        aria-label={text({
                          en: `Remove ${photo.name}`,
                          es: `Eliminar ${photo.name}`,
                        })}
                        className="absolute right-2 top-2 inline-flex size-8 items-center justify-center rounded-full border border-border bg-card/95 text-ink shadow-soft"
                      >
            ...
            {ready && (
              <p
                className="rounded-lg bg-accent/40 p-4 text-sm leading-relaxed text-accent-foreground"
                role="status"
              >
                {text({
          ...
          <aside className="rounded-xl border border-border bg-sand p-6 lg:sticky lg:top-32">
            ...
            <h2 className="mt-3 text-xl">
              {text({ en: "Prefer to talk it through?", es: "¿Prefieres hablarlo directamente?" })}
            </h2>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              {text({
                en: "Call or email directly and we will walk through the space with you.",
                es: "Llámanos o envíanos un correo y revisaremos el espacio contigo.",
              })}
            </p>
            <div className="mt-5 flex flex-col gap-2">
              ...
              <Button asChild variant="outline">
                <a href={business.emailHref}>
                  {text({ en: "Email us", es: "Envíanos un correo" })}
                </a>
              </Button>
            </div>
          </aside>
